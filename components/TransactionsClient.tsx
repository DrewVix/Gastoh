...
import { Search, ChevronLeft, ChevronRight, Pencil, Check, X, Plus, Store } from 'lucide-react'
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between text-sm px-1" style={{ color: 'var(--muted)' }}>
        <span>{total} transacciones{filterMonth || filterCategory || q || filterMerchant ? ' · filtradas' : ''}</span>
        {totalPages > 1 && (
      ...
      {showNewModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div className="card w-full max-w-md p-6 space-y-4" style={{ background: 'var(--card)', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <form onSubmit={createTransaction} className="space-y-3">
              ...
              <div className="flex gap-2 pt-1">
                <button type="button" onClick={() => setShowNewModal(false)}
                  className="flex-1 text-sm py-2 rounded-lg transition-colors hover:opacity-80"
                  style={{ border: '1px solid var(--card-border)', color: 'var(--muted)' }}>
                  Cancelar
                </button>
                <button type="submit" disabled={newSaving}
                  className="flex-1 text-sm py-2 rounded-lg transition-opacity disabled:opacity-50"
                  style={{ background: 'var(--accent)', color: '#fff' }}>
